...
def generate_import_dependency_tables(drug_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for item in trade_data:
            ...
            dependency_percent = (volume_q3 / total_volume * 100) if total_volume > 0 else 0
            ...
            dependency_table.append({
                ...
                "stability_score": calculate_stability_score(item.get("qoq_growth", "0%"))
            })
        ...
        if len(price_data) > 1:
            ...
            price_volatility = "HIGH" if abs(price_change_percent) > 10 else "MODERATE" if abs(price_change_percent) > 5 else "LOW"
        else:
        ...
        concentration_index = sum((item["dependency_percent"] ** 2) for item in dependency_table)
        ...
        return {
            ...
            "import_risk_assessment": {
                ...
                "details": import_risks_info.get("details", "No risk details available"),
                "concentration_index": round(concentration_index, 1),
                "top_risk_countries": [d for d in dependency_table if d["dependency_risk"] in ["CRITICAL", "HIGH"]],
                "overall_risk_level": assess_overall_risk(concentration_index, len(trade_data))
            },
            "supply_chain_resilience": {
                "resilience_score": resilience_score,
                "num_suppliers": len(trade_data),
                "primary_supplier_dependency": max([d["dependency_percent"] for d in dependency_table]),
                "supply_diversity": "EXCELLENT" if len(trade_data) >= 5 else "GOOD" if len(trade_data) >= 3 else "LIMITED" if len(trade_data) == 2 else "SINGLE SOURCE",
...
def calculate_resilience_score(num_suppliers: int, dependency_table: List[Dict]) -> str:
    ...
    actual_dependency = dependency_table[0]["dependency_percent"] if dependency_table else 100
    distribution_score = min(100, (avg_dependency / actual_dependency) * 100) if actual_dependency > 0 else 0

...
def extract_sourcing_insights(drug_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for supplier in suppliers:
            share = (supplier["volume_q3_2024"] / total_volume * 100) if total_volume > 0 else 0
        ...
        supplier_share_sorted = sorted(supplier_share, key=lambda x: x["market_share_percent"], reverse=True)
        ...
        for region, countries in regions.items():
            if region != "Others":
                volume = sum(s["volume_q3_2024"] for s in suppliers if any(c in s["country"] for c in countries))
                if volume > 0:
                    regional_breakdown[region] = {
                        "volume": volume,
                        "share_percent": round((volume / total_volume * 100), 1) if total_volume > 0 else 0,
                        "countries": [s["country"] for s in suppliers if any(c in s["country"] for c in countries)]
                    }
        ...
        if hhi_index > 5000:
            recommendations.append("CRITICAL: Diversify supplier base to reduce supply chain risk")
        
        top_supplier_share = supplier_share_sorted[0]["market_share_percent"] if supplier_share_sorted else 0
        if top_supplier_share > 50:
            recommendations.append(f"Explore alternative suppliers - Top supplier has {top_supplier_share}% market share")
        
        if len(suppliers) == 1:
            recommendations.append("Single-source dependency - Establish backup suppliers")
        ...
        if growth_data:
            ...
            recommendations.append(f"Fastest growing market: {fastest_growing[0]} ({fastest_growing[1]:.1f}% QoQ growth)")

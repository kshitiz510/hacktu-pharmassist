...
def analyze_trade_volumes(drug_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sorted_countries = sorted(
            trade_data, key=lambda x: x.get("q3_2024", 0), reverse=True
        )
        ...
        return {
            ...
            "summary": {
                ...
                "volume_growth": f"+{((total_q3_volume - total_q2_volume) / total_q2_volume * 100):.1f}%"
                if total_q2_volume > 0
                else "N/A",
                "average_qoq_growth": f"{avg_growth:.1f}%",
                "top_supplier_country": sorted_countries[0]["country"]
                if sorted_countries
                else "N/A",
                "top_supplier_volume": sorted_countries[0].get("q3_2024", 0)
                if sorted_countries
                else 0,
            },
            "raw_data": trade_data,

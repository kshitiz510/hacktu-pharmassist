...
export default function NewsMonitorPage({ onBack, onSelectChat, onRefreshMonitored }) {
  ...
  const handleBroadcastIntel = async () => {
    if (!intelText.trim() || isBroadcasting) return;
    setIsBroadcasting(true);
    setBroadcastResult(null);
    try {
      const result = await api.broadcastIntel(intelText.trim());
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-card border border-border rounded-xl pl-9 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/20 transition-all"
              disabled={isBroadcasting}
            />
          </div>
          <button
            onClick={handleBroadcastIntel}
            disabled={!intelText.trim() || isBroadcasting}
          ...
          >
            ...
            {isBroadcasting ? 'Checking...' : 'Broadcast'}
          </button>
        </div>
        <p className="text-[11px] text-muted-foreground mt-1.5 ml-1">
          Runs the news comparator against all monitored chats with your new intel
        </p>
      </div>
    </div>
  );
